...
def generate_cyclonedx_sbom(pyproject: str | Path, *, source_commit: str = "UNKNOWN") -> dict[str, Any]:
    ...
    components = [
        {
            ...
            "properties": [{"name": "rpos:declared-requirement", "value": requirement}],
        }
    ...
    return {
        ...
        "metadata": {
            ...
            "tools": {"components": [{"type": "application", "name": "rpos-sbom", "version": "1"}]},
            "component": {
                "type": "application",
                "name": name,
                "version": version,
                "properties": [{"name": "rpos:source-commit", "value": source_commit}],
            },
        },
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate CycloneDX JSON SBOM from RPOS project metadata.")
    ...
    Path(args.output).write_text(
        json.dumps(generate_cyclonedx_sbom(args.pyproject, source_commit=args.source_commit), indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    return 0
