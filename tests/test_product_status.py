...
def test_product_status_matches_package_identity_and_release_boundary() -> None:
    ...
    assert status["release_stage"] == "early_public_alpha_release_candidate"
    assert status["publication_state"] == "not_published"
    assert status["latest_published_version"] == "0.1.0a1"
    ...
    assert "pypi_publication_via_trusted_publisher" not in surfaces
    ...
    assert signature["lineage"] == "independently engineered Responsibility Pathway lineage"
